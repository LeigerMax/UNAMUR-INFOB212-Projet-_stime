...
class ObjetInstance:
    ...
    @classmethod
    @with_connection
    def select_all_from_user(cls, user_id, **kwargs):
        # get cursor from connection in kwargs
        cursor = get_cursor(kwargs)

        # execute query
        query = "SELECT * FROM OBJET_INSTANCE WHERE Possesseur = %s"
    ...
    def update(cls, objet_instance, **kwargs):
        ...
        # execute query
        query = "UPDATE OBJET_INSTANCE SET DateObtention = %s, Possesseur = %s, Objet = %s, Panier = %s WHERE Id = %s"
